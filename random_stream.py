...
# from prefix_codes import delta_coding, decode_delta_coding
...
def run_length_huffman_core(quantized_grads, prev_count=0, magnitude = 0):
	...
	encode_time_s = time.time()
	# encoded_doc = []
	encoded_doc = torch.zeros([1500000,],  dtype=torch.int8)
	count = prev_count
...
def get_tensor(swap_encodings, encoded_doc):
	...
	l = torch.zeros([15000,],  dtype=torch.int8)
	count = 0
	...
	# print("e/xiting")
	# exit(0)
	return swap_encodings, third_tensor


def decode_RLH(padded_encoded_grad, size, max_len=1000000):
...
def decode_generic(padded_encoded_grad, size, max_len=1000000):
	## Elements in padded_encoded_grad are valid only upto index size-1
	# print(len(padded_encoded_grad), padded_encoded_grad)
	idx = 0
	swap_encodings = {}
...
def decode_generic_sparse(padded_encoded_grad, size, max_len=1000000):
	## Elements in padded_encoded_grad are valid only upto index size-1
	# print(len(padded_encoded_grad), padded_encoded_grad)
	idx = 0
	swap_encodings = {}
...
def decode_omega(encoded_doc, size, magnitude, max_len=1000000):
...
# encoded_doc = d[3]
# decode_omega(encoded_doc)
filename = "r0_gradients_iter_42.npy"


quantized_grads, magnitude = get_grad(filename)
print(magnitude)
# exit(0)
quantized_grads = quantized_grads.astype(int)
print(quantized_grads[:10])
d = Run_Length_Encode_efficient_16bit_omega(quantized_grads)

encoded_doc = d[3]
decode_omega(encoded_doc, len(encoded_doc),24)
print(elias_omega_enc)
exit(0)
unique_qstates,counts_qstates = np.unique(quantized_grads, return_counts=True)
# document, run_code_length, encodings, swap_encodings, encoded_doc  = run_length_huffman_core(quantized_grads,0, magnitude)
swap_encodings, encoded_doc = sample_huffman_with_sparsity(quantized_grads)
# exit(0)

se, third_tensor = get_tensor(swap_encodings, encoded_doc )
i=0
# for key in se:
#     # print(key, se[key])
#     i+=1
#     if(i==10):
#         break
decoded_quantized_grad = decode_generic_sparse(third_tensor,third_tensor.numel())
print(len(quantized_grads), "\n",quantized_grads[:10])
print(len(decoded_quantized_grad),"\n", decoded_quantized_grad[:10])
# exit(0)
# print(len(se.keys()), len(dec_se.keys()))
# exit(0) 
# flag = 1
# for key in se:
#     if se[key]!= dec_se[key]:
#         flag=0
#         print("Error")
# if (flag==1):
print("Success")
...
# print(counts_qstates)
# document, doc_len, unique_elements, counts_elements  =  Run_Length_Encode(quantized_grads)
base_code_length  = base_huffman(quantized_grads)
sample_sparse_code_length = sample_huffman_with_sparsity_efficient_old(quantized_grads)
###  16 BIT ########### ########### ########### ########### 
bit_enc_16 = Run_Length_Encode_efficient_16bit(quantized_grads)
# start_time = time.time()
# document, doc_len, frequency  =  Run_Length_Encode_efficient(quantized_grads)
# encodings = {}
# for i in range(256):
...
# "".join(encoded_doc)
# end_time = time.time()
# print ("16-bit encoding L :",(16+16)*doc_len, len(encoded_doc), "Time: ", end_time - start_time)
########### ########### ########### ########### ########### ########### ########### 
# print(document)
elias_omega_enc = Run_Length_Encode_efficient_16bit_delta(quantized_grads)
elias_delta_enc = Run_Length_Encode_efficient_16bit_omega(quantized_grads)
run_code_length = run_length_huffman(quantized_grads)
sample_run_code_length = sample_run_length_huffman(quantized_grads)
sample_no_sparse_code_length  = sample_huffman_no_sparsity(quantized_grads)
sample_sparse_code_length = sample_huffman_with_sparsity(quantized_grads)

base_code_length  = base_huffman(quantized_grads)


### Talking Points
### All the iterations have similar sign change differences
### The percentage change is more or less  the same even between the iteration 380 and 11000
